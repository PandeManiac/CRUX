...
extern "C"
{
	...
	int mtx_timedlock(mtx_t* mtx, const struct timespec* ts)
	{
		...
		else
		{
			...
			timeoutMs += (unsigned)(ts->tv_nsec - current_ts.tv_nsec) / 1000000;
			timeoutMs += 1;
		}
...
#define _CONDITION_EVENT_ALL 1
	...
	int cnd_timedwait(cnd_t* cond, mtx_t* mtx, const struct timespec* ts)
	{
		...
		{
			unsigned long long nowInMilliseconds = (unsigned)now.tv_sec * 1000 + (unsigned)now.tv_nsec / 1000000;
			unsigned long long tsInMilliseconds	 = (unsigned)ts->tv_sec * 1000 + (unsigned)ts->tv_nsec / 1000000;
